...
public class TelaConta extends javax.swing.JDialog {
   ...
   protected void atualizaTabela() {
     try {
        ...
        lista = new ContaDao().getLista(txPesquisaCat.getText());
        double saldoTotalMovimentacoes = 0;
        
        for (Conta c : lista) {
            saldoTotalMovimentacoes = new MovimentacaoDao().getSaldoTotalPorConta(c.getIdConta());

            model.addRow(new Object[]{
                c.getIdConta(),
                c.getNomeConta(),
                saldoTotalMovimentacoes
            });
        }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        TelaContaCadastro t = new TelaContaCadastro(this, true);
        t.setInserir(true);
        t.setVisible(true);
    }//GEN-LAST:event_jButton1ActionPerformed

    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        if(tabelaConta.getSelectedRow()>=0){
        TelaContaCadastro t = new TelaContaCadastro(this, true);
        t.setInserir(false);
        t.preencheCampos(lista.get(tabelaConta.getSelectedRow()));
        t.setVisible(true);
        }else{
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        if (tabelaConta.getSelectedRow() >= 0) {
        Conta contaSelecionada = lista.get(tabelaConta.getSelectedRow());

        // Verificar se há movimentações associadas à conta
        double saldoTotalMovimentacoes = new MovimentacaoDao().getSaldoTotalPorConta(contaSelecionada.getIdConta());
        
        if (saldoTotalMovimentacoes > 0) {
            Msg.alert(this, "Esta conta possui movimentações associadas e não pode ser excluída!");
        } else {
            if (Msg.confirmar(this, "Deseja realmente excluir esse registro?")) {
                new ContaDao().excluir(contaSelecionada);
                Msg.informacao(this, "Excluído com sucesso!");
                atualizaTabela(); // Atualizar a tabela após a exclusão
            }
        }
    } else {
        Msg.alert(this, "Selecione um registro!");
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                TelaConta dialog = new TelaConta(new javax.swing.JFrame(), true);
